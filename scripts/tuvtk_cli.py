...
class LinuxDockerBackend:
    ...
    def setup(self, mode: str, *, domain: str = "", port: int = 8000) -> None:
...
Usage: ./install.sh COMMAND [ARGS...]
       .\\install.ps1 COMMAND [ARGS...]     (Windows PowerShell)

Bootstrap and lifecycle:
  dev [--port=PORT]              Run development in the current terminal
  setup [dev|production]         Prepare without starting application services
  deploy --domain=HOST           Prepare, build, and start Debian production
  start, stop, restart, status   Manage the configured stack
  build, rebuild, logs [SERVICE] Build or inspect the configured stack
...
Data and maintenance:
  ...
  export-sql PATH
  import-sql FILE [--yes]
  dev-db-reset [--yes] [--start]
  context [ARGS...]
  clean

Legacy installer flags such as --dev and --production remain supported on Linux.
Windows supports native development; production deployment is Debian/Docker only.
"""
...
def main(arguments: list[str]) -> int:
    ...
    if command == "setup":
        ...
        port = validate_port(int(option_value(rest, "--port", str(config.get("dev_port", 8000)))))
        if IS_WINDOWS:
            ...
            backend.setup(requested_mode, domain=option_value(rest, "--domain"), port=port)
        return 0
    if command in {"dev", "dev-start"}:
        ...
        if IS_WINDOWS:
            ...
            if not DEV_ENV_FILE.exists() or port != int(config.get("dev_port", -1)):
                backend.setup("dev", port=port)
            backend.invoke("dev", [], "dev")
        return 0
    if command == "deploy":
        if IS_WINDOWS:
            raise CliError("production deployment is supported only on Debian/Docker")
        domain = option_value(rest, "--domain")
        backend.setup("prod", domain=domain)
